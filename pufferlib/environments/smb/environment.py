...
def env_creator(name='SuperMarioBros-1-1-v1'):
...
def make(name, buf=None, seed=None, render_mode='rgb_array', **kwargs):
    ...
    env = gymnasium.wrappers.TimeLimit(env, max_episode_steps=(400-277)*25)
    ...
    env = pufferlib.ResizeObservation(env, downscale=4)
    env = gymnasium.wrappers.FrameStack(env, 4)
    
    from os import getenv
    if getenv('DEBUG', '0') == '1':
        env = RenderObservation(env, framestack=True) # for debugging
...
class RenderObservation(gymnasium.ObservationWrapper):
    ...
    def __init__(self, env, framestack=False):
        super().__init__(env)
        self.last_obs = None
    ...
    def render(self):
        ...
        if self.framestack:
            # convert to grayscale image with each channel side-by-side
            return np.concatenate(self.last_obs, axis=1)
